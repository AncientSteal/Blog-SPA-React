[![CI (Code Quality and Build)](https://github.com/AncientSteal/Blog-SPA-React/ci.yml/badge.svg)](https://github.com/AncientSteal/Blog-SPA-React/ci.yml/badge.svg) 
...
В проекте настроено автоматическое тестирование и проверка качества кода (CI) с помощью **GitHub Actions**. Робот запускается при каждом пуше в ветку `main`.

### Как запустить тесты локально:

1. **Запустить тесты в режиме разработки (Watch mode):**
   (В этом режиме Vitest автоматически перепроверяет тесты при изменении кода)
   ```bash
   npm run test
   ```

2. **Запустить одиночную проверку тестов (Режим CI):**
   ```bash
   npm run test:ci
   ```

3. **Запустить проверку линтера на ошибки оформления:**
   ```bash
   npm run lint
   ```
